...
def _cache_get(cache, server, lock):
    with lock:
        return cache.get(server)


def _cache_set(cache, server, value, lock):
    with lock:
        cache[server] = value
    return value


def _get_nation_info(server, nation_cache, lock):
    ...
    if not ip:
        ...
        return _cache_set(nation_cache, server, ("未知", "cn", 0), lock)
    ...
    if pair:
        ...
        return _cache_set(nation_cache, server, (nation, iso_code, 0), lock)
...
def fetch_all_nations(domains, ips):
    ...
    def thread_func(server):
        return _get_nation_info(server, nation_cache, lock)
